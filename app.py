...
# --- SUB-ABA: VISUALIZAR REGISTROS ---
        ...
        with tab_oc1:
            ...
            # Código de fechamento e transição exata para substituir o erro
            with st.form("form_ocorrencia", clear_on_submit=True):
                ...
                tipo_selecao_oc = st.multiselect("Selecione as ocorrências", options=opcoes_ocorrencias)
                obs_oc = st.text_area("Detalhes/Observações adicionais")
                
                col_salvar_oc, col_msg_oc = st.columns([1, 2])
                with col_salvar_oc:
                    btn_salvar_oc = st.form_submit_button("GRAVAR OCORRÊNCIA", disabled=is_soe)

            if btn_salvar_oc:
                if not tipo_selecao_oc:
                    with col_msg_oc:
                    ...
                    try:
                        sh = conectar_google_sheets()
                        wks = sh.worksheet("Registros_Ocorrencias")
                        tipo_formatado = "OCORRÊNCIA: " + ", ".join(tipo_selecao_oc)
                        detalhes_formatados = f"DATA: {data_ocorrido.strftime('%d/%m/%Y')} | TEMPO: {tempo_aula} | OBS: {obs_oc}"
                        
                        nova_linha = [
                            datetime.now(fuso_roraima).strftime("%d/%m/%Y %H:%M:%S"),
                            prof_nome, turma_sel, aluno_sel, disciplina, periodo,
                            tipo_formatado, detalhes_formatados
                        ]
                        wks.append_row(nova_linha)
                        with col_msg_oc:
        ...
        with tab_oc2:
            ...
            try:
                ...
                dados_brutos = wks_reg.get_all_values()
                if len(dados_brutos) > 1:
                    df_oc = pd.DataFrame(dados_brutos[1:], columns=dados_brutos[0])
                    df_oc['ID_Original'] = range(2, len(df_oc) + 2)
                    df_oc = df_oc[df_oc[df_oc.columns[6]].astype(str).str.contains("OCORRÊNCIA:", na=False)]
                    ...
                    with col_fo1:
                        bim_filtro_oc = st.selectbox("Filtrar por Bimestre (Ocorrências)", ["Todos"] + sorted(df_oc[col_bim_oc].unique().astype(str).tolist()))
                    with col_fo2:
                        if st.session_state.get('is_master_admin', False) or is_soe:
                    ...
                    df_oc_exibicao = df_oc_filtrado[[col_turma_oc, colunas_df[3], col_bim_oc, col_disc_oc, colunas_df[1], colunas_df[6], colunas_df[7]]]
                    st.dataframe(df_oc_exibicao, use_container_width=True, hide_index=True)
                else:
        ...
        with st.form("form_senha_propria"):
            ...
            btn_alterar_senha_prof = st.form_submit_button("ATUALIZAR SENHA")
            
            if btn_alterar_senha_prof:
                if str(nova_senha_prof).strip() != str(confirmar_senha_prof).strip():
                    st.error("As senhas não conferem.")
                elif str(nova_senha_prof).strip() == "":
