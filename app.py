...
# --- BLOCO ADICIONAL (ÂNCORAS NÃO LOCALIZADAS) ---
        ...
        with tab1:
            # Lógica de Turmas vinculadas ao Professor Logado
            if st.session_state.user_data['Usuario'] in ["admin", "rodrigo"]:
                todas_turmas_ag = sorted(df_alunos['Turma'].unique().astype(str))
            else:
                # Busca turmas do dicionário de dados do usuário logado
            ...
            with st.form("form_agendamento", clear_on_submit=True):
                ...
                # Novo campo de seleção de Turma integrado
                turma_agendada = st.selectbox("Selecione a Turma que usará o equipamento", todas_turmas_ag)
                    ...
                    if verificar_conflito(equipamento, data_str, turno, horario):
                        ...
                        try:
                            ...
                            nova_linha_ag = [
                                ...
                                data_str,
                                turno,
                                f"{horario} - Turma: {turma_agendada}", # Salva a turma junto ao horário
                                obs_ag
                            ]
                            wks_ag.append_row(nova_linha_ag)
                            st.success(f"✅ {equipamento} agendado com sucesso para a turma {turma_agendada}!")
                            time.sleep(2)
                            st.rerun()
                        except Exception as e:
        ...
        with tab2:
            ...
            if not df_lista.empty:
                ...
                # Exibe a tabela com os agendamentos existentes
