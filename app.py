...
# --- BLOCO ADICIONAL (ÂNCORAS NÃO LOCALIZADAS) ---
        ...
        with tab1:
            ...
            if st.session_state.user_data['Usuario'] in ["admin", "rodrigo"]:
                ...
                # Pega as turmas vinculadas ao professor logado
            ...
            if not todas_turmas_ag:
                st.warning("Você não possui turmas vinculadas para realizar agendamentos.")
            else:
                with st.form("form_agendamento", clear_on_submit=True):
                    ...
                    # Seleção da Turma (mesma lógica dos professores)
                    ...
                    if btn_agendar:
                        ...
                        if verificar_conflito(equipamento, data_str, turno, horario):
                            ...
                            try:
                                ...
                                st.success(f"✅ {equipamento} agendado para a turma {turma_agendada}!")
                                time.sleep(2)
                                st.rerun()
                            except Exception as e:
                                st.error(f"Erro ao salvar: {e}")
