...
if 'usuario_perfil' not in st.session_state:
    st.session_state.usuario_perfil = None # Default to None or "professor"
# Initialize maintenance states
if 'maint_Registro' not in st.session_state:
    st.session_state.maint_Registro = False
if 'maint_Ocorrencias' not in st.session_state:
    st.session_state.maint_Ocorrencias = False
if 'maint_Agendamentos' not in st.session_state:
    st.session_state.maint_Agendamentos = False
if 'maint_Seguranca' not in st.session_state:
    st.session_state.maint_Seguranca = False
...
if not st.session_state.logado:
        ...
        if entrar:
            ...
            if user_input.lower() == 'admin':
                ...
                if str(pass_input).strip() == senha_admin_planilha:
                    ...
                    st.session_state.usuario_perfil = "adm master" # NEW: Set profile
                ...
                if not match.empty:
                    ...
                    if "Status" in user_row and str(user_row["Status"]).upper() == "BLOQUEADO":
                        ...
                        st.session_state.usuario_perfil = "professor" # NEW: Set profile
    ...
    # PAINEL DE CONTROLE EXCLUSIVO DO ADM MASTER
    if st.session_state.get('usuario_perfil') == "adm master": # Using the new profile check
        st.sidebar.markdown("---")
        st.sidebar.subheader("🛠️ Manutenção do Sistema")
        st.session_state.maint_Registro = st.sidebar.toggle("🔒 Bloquear Registro (Notas/Desempenho)", value=st.session_state.maint_Registro, help="Ative para bloquear o formulário de registro de desempenho para usuários comuns.")
        st.session_state.maint_Ocorrencias = st.sidebar.toggle("🔒 Bloquear Ocorrências", value=st.session_state.maint_Ocorrencias, help="Ative para bloquear o formulário de ocorrências para usuários comuns.")
        st.session_state.maint_Agendamentos = st.sidebar.toggle("🔒 Bloquear Agendar Equipamentos", value=st.session_state.maint_Agendamentos, help="Ative para bloquear o formulário de agendamento de equipamentos para usuários comuns.")
        st.session_state.maint_Seguranca = st.sidebar.toggle("🔒 Bloquear Segurança", value=st.session_state.maint_Seguranca, help="Ative para bloquear a página de segurança (alteração de senha) para usuários comuns.")
        st.sidebar.markdown("---")
    ...
    if st.sidebar.button("Sair", key="btn_sair", use_container_width=True):
        ...
        st.session_state.usuario_perfil = None # NEW: Reset profile on logout
        st.session_state.pagina = "Registro"
        st.rerun()
    ...
    # --- Controle da Página Desempenho / Registro ---
    if pagina_atual == "Registro":
        if st.session_state.maint_Registro and st.session_state.get('usuario_perfil') != "adm master":
            st.title("📊 Desempenho do Aluno") # Keep original title
            st.info("⚠️ **Recurso em Manutenção Preventiva**\n\nA área de registros e notas está temporariamente fechada para atualizações. Retornaremos em breve!")
        else:
            if st.session_state.get('usuario_perfil') == "adm master" and st.session_state.maint_Registro:
                st.warning("⚡ **Aviso Master:** O painel de registros está bloqueado para os professores.")
            # Original content of "Registro" page starts here
    ...
    # --- Controle da Página Ocorrências ---
    elif pagina_atual == "Ocorrências":
        if st.session_state.maint_Ocorrencias and st.session_state.get('usuario_perfil') != "adm master":
            st.title("🚨 Registro de Ocorrências") # Keep original title
            st.info("⚠️ **Recurso em Manutenção Preventiva**\n\nO registro de ocorrências está indisponível no momento devido a atualizações na ferramenta.")
        else:
            if st.session_state.get('usuario_perfil') == "adm master" and st.session_state.maint_Ocorrencias:
                st.warning("⚡ **Aviso Master:** O painel de ocorrências está bloqueado para os professores.")
            # Original content of "Ocorrências" page starts here
    ...
    # --- Controle da Página Segurança ---
    elif st.session_state.pagina == "Segurança":
        if st.session_state.maint_Seguranca and st.session_state.get('usuario_perfil') != "adm master":
            st.title("🔒 Segurança") # Keep original title
            st.info("⚠️ **Recurso em Manutenção Preventiva**\n\nA área de alteração cadastral e senhas está em manutenção.")
        else:
            if st.session_state.get('usuario_perfil') == "adm master" and st.session_state.maint_Seguranca:
                st.warning("⚡ **Aviso Master:** O painel de segurança está bloqueado para os professores.")
            # Original content of "Segurança" page starts here
            st.title("🔒 Segurança")
    ...
    # --- Controle da Página Agendar Equipamentos ---
    elif pagina_atual == "Agendamento de Equipamentos": # Keep original page name
        if st.session_state.maint_Agendamentos and st.session_state.get('usuario_perfil') != "adm master":
            st.title("📅 Gerenciamento de Equipamentos") # Keep original title
            st.info("⚠️ **Recurso em Manutenção Preventiva**\n\nO formulário de agendamento de equipamentos está pausado para melhorias técnicas.")
        else:
            if st.session_state.get('usuario_perfil') == "adm master" and st.session_state.maint_Agendamentos:
                st.warning("⚡ **Aviso Master:** O agendamento de equipamentos está bloqueado para os professores.")
            # Original content of "Agendamento de Equipamentos" page starts here
            ...
            with aba_cadastrar:
                ...
                # The original internal check for 'bloqueio_agendamento' is removed here.
                # The main page check handles the blocking.
