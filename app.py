...
# =============================================================================
# FUNÇÕES DE TELAS ISOLADAS E INDEPENDENTES (BLINDAGEM DE MÓDULOS)
# =============================================================================

def desenhar_tela_ocorrencias(prof_nome, is_soe, df_periodos, df_alunos, data_atual, fuso_roraima):
    ...
    with tab_oc1:
        ...
        if btn_salvar_oc:
            if is_soe:
                ...
                try:
                    # Conexão isolada e segura do módulo de ocorrências
                    scope = ["https://www.googleapis.com/auth/spreadsheets", "https://www.googleapis.com/auth/drive"]
                    creds_dict = st.secrets["gcp_service_account"]
                    credentials = Credentials.from_service_account_info(creds_dict, scopes=scope)
                    client = gspread.authorize(credentials)
...
def desenhar_tela_agendamentos():
    st.title("📅 Agendamento de Equipamentos")
    st.markdown("---")
    # Este bloco está totalmente independente e pronto para começarmos a criar do zero
    st.info("🔄 Este módulo de agendamento está 100% isolado em uma função própria e pronto para ser reestruturado do zero, sem risco de interferir em outros botões.")
