...
try:
    sh = conectar_google_sheets() # Conecta uma vez para carregar dados
...
# --- SUB-ABA: VISUALIZAR REGISTROS ---
    ...
    # =========================================================================
        ...
        # Criação das sub-abas mantendo rigorosamente os seus nomes originais
        aba_criar, aba_correcao, aba_historico = st.tabs(["Criar", "Correção", "Histórico de notas"])

        with aba_criar:
            st.markdown("### Configurar Nova Avaliação e Cartão Resposta")
            
            # Busca as listas de professores, turmas e o bimestre ativo definido pelo ADM
            try:
                ...
                # Tentativa de buscar o bimestre ativo determinado pelo administrador
                try:
                    df_escola = pd.DataFrame(sh_aux.worksheet("Config_Escola").get_all_records())
                    bimestre_ativo = df_escola["Bimestre_Ativo"].iloc[0] if "Bimestre_Ativo" in df_escola.columns else "1º Bimestre"
                except Exception:
                    bimestre_ativo = "1º Bimestre"
            except Exception:
            ...
            with st.form("form_criar_gabarito_camera", clear_on_submit=True):
                ...
                with col_m2:
                    # O professor seleciona do 6º ao 9º ano
                    serie_selecionada = st.selectbox("Série:", ["6º ano", "7º ano", "8º ano", "9º ano"])

                col_n1, col_n2 = st.columns(2)
                with col_n1:
                    # Campo de Bimestre vindo preenchido automaticamente de acordo com o ADM (apenas visualização)
                    bimestre_selecionado = st.text_input("Bimestre Atual (Definido pelo Adm):", value=str(bimestre_ativo), disabled=True)
                with col_n2:
                ...
                st.write("---")
                btn_salvar_exportar = st.form_submit_button("Gerar e exportar folha de prova com cartão resposta", use_container_width=True)

                if btn_salvar_exportar:
                    try:
                        # Conexão e salvamento no Google Sheets
                        sh = conectar_google_sheets()
                        try:
                            wks_avaliacoes = sh.worksheet("Config_Avaliacoes")
                        except gspread.exceptions.WorksheetNotFound:
                            wks_avaliacoes = sh.add_worksheet(title="Config_Avaliacoes", rows="1000", cols="9")
                            wks_avaliacoes.append_row(["ID_Avaliacao", "Nome_Avaliacao", "Professor_Disciplina", "Serie", "Bimestre", "Qtd_Questoes", "Qtd_Alternativas", "Gabarito_JSON", "Data_Criacao"])

                        # Geração do ID único por timestamp
                        ...
                        # Gravação dos dados mantendo o controle do Bimestre e da Série no banco
                        ...
                        # Cabeçalho impresso: Série e Bimestre preenchidos, Turma e Nome vazios para o aluno fazer à mão
                        st.markdown("### 🖨️ Pré-visualização da Folha de Resposta Pronta para Impressão")
                        st.code(
                            f"=========================================================================\n"
                            f"ESCOLA ESTADUAL PROFESSORA DIVA ALVES DE LIMA\n"
                            f"PROFESSOR(A): {prof_selecionado} | BIMESTRE: {str(bimestre_selecionado).upper()}\n"
                            f"SÉRIE: {serie_selecionada.upper()} | TURMA: ___________________ \n"
                            f"NOME DO ALUNO: _________________________________________________________\n"
